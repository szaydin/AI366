## Student: Sumeyye Aydin
## I run the program with alpha = 0.1 and 200 for 500 runs
## I got these result
## Average return: -239.385
## Overall Performance : -49310.172
## Mean performance: -246.55086
## Standard Error: 0.40745719201
...
def learn(alpha=.4 / numTilings, epsilon=0, numEpisodes=200):
    theta1 = -0.001 * rand(n)
    theta2 = -0.001 * rand(n)
    returnSum = 0.0
    Qs1 = np.zeros(3)
    ...
    for episodeNum in range(numEpisodes):
        ...
        while s != None:
            ...
            if p == 0:
                a_ss = np.argmax([ qvalues(theta1,0, t2), qvalues(theta1, 1, t2), qvalues(theta1, 2, t2)] )
                Q_ss = qvalues(theta2, a_ss,t2)
                for i in t1:
                    theta1[i + a * 324] += alpha * (r + Q_ss - Qs1[a])
            if p == 1:
                a_ss = np.argmax([ qvalues(theta2, 0, t2), qvalues(theta2, 1, t2), qvalues(theta2, 2, t2)])
